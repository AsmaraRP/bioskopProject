...
module.exports = {
  getTotalMovie: (searchName, searchRelease) =>
    new Promise((resolve, reject) => {
      connection.query(`SELECT COUNT(*) AS total FROM movie WHERE name LIKE '%${searchName}%' && releaseDate LIKE '%${searchRelease}%'`, (error, result) => {
        if (!error) {
  ...
  getAllMovie: (limit, offset, searchName, sort, searchRelease) =>
    new Promise((resolve, reject) => {
      connection.query(`SELECT * FROM movie WHERE name LIKE '%${searchName}%' && releaseDate LIKE '%${searchRelease}%' ORDER BY ${sort} LIMIT ${limit} OFFSET ${offset}`, (error, result) => {
        if (!error) {
          resolve(result);
        } else {
